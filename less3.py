...
class GbBlogParse:
    ...
    def _get_soup(self, url, *args, **kwargs) -> bs4.BeautifulSoup:
        soup = bs4.BeautifulSoup(self._get_response(url, *args, **kwargs).text)
        return soup
    ...
    def parse_post(self, url, soup):
        ...
        data = {
            ...
            "tags_data": [urljoin(url, tag_a.attrs.get("href")) for tag_a in soup.find_all('a', attrs={'class': 'small'})],
        }
        return data

    def parse_feed(self, url, soup):
        ul = soup.find('ul', attrs={"class": "gb__pagination"})
        pag_urls = set(urljoin(url, url_a.attrs.get('href'))
                       for url_a in ul.find.all("a") if url_a.attrs.get('href'))
        ...
        post_urls = set(
            urljoin(url, url_a.attrs.get('href')) for url_a in soup.find.all(
                'a', attrs={"class": "post-item__title"}) if url_a.attrs.get('href')
        )
